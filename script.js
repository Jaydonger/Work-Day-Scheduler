...
var today = dayjs().format('DD/MM/YYYY, h a');
...
$(document).ready(function () {
    ...
    $(".saveBtn").on('click', function() {
        ...
        if(dayjs().isBefore(dayjs().hour(`${hour}`))){
            $(`#${hour}`).addClass('future').removeClass('past').removeClass('present');
        }else if(dayjs().isSame(dayjs().hour(`${hour}`))){
            $(`#${hour}`).addClass('present').removeClass('past').removeClass('future');
        }else{ 
            $(`#${hour}`).addClass('past').removeClass('present').removeClass('future');
        }
        var ppfVal = $(this).parent().attr('class').split(' ');

        console.log(ppfVal.filter(e => {return e === 'past' || e === 'present' || e === 'future'}));
        var dataObj = {
